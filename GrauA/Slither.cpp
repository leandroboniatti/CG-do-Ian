...
int main() {
	...
	GLFWwindow* window = glfwCreateWindow(WIDTH, HEIGHT, "SIMULANDO A COBRINHA DO SLITHER.IO - Ian R. Boniatti", nullptr, nullptr);	// Criação da janela GLFW
	...
	float passo = 0.1;	// incremento na posição x ou y
	double cursorPosX;	double cursorPosY;	// para guardar a posição devolvida por glfwGetCursorPos

	for (int i=0; i < 1024; i++) {keys[i]=false;}	// inicialização de todas com false
